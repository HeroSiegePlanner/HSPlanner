...
mod tests {
    ...
    fn decimal_values() {
        ...
        assert_eq!(parse_custom_stat_value("12.345"), Some((12.345, 12.345)));
    ...
    fn equal_endpoints_kept_as_pair() {
        // TS collapses min === max to a scalar, Rust normalises to Ranged; both
        // round-trip to the same downstream value, so parity holds.
        assert_eq!(parse_custom_stat_value("12-12"), Some((12.0, 12.0)));
    }

    #[test]
    fn matches_fixture_inputs() {
        // Same three strings the class_with_custom_stat parity fixture feeds through
        // parseCustomStatValue, pinning Rust output against the TS baseline.
