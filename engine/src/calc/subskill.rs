...
use super::conditions::{is_condition_active, is_conditional_key};
...
/// Effective tags after subskill transmutations (removes win over adds).
/// Mirrored in frontend utils/skills/skillTags.ts:effectiveSkillTags.
pub fn effective_skill_tags(
    skill_id: &str,
    base: &[String],
    subskill_ranks: &HashMap<String, u32>,
) -> Vec<String> {
    let Some(changes) = crate::calc::data::subskill_tag_changes(skill_id) else {
        return base.to_vec();
    };
