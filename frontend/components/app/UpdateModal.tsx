...
function ChangelogBlock({ section }: { section: ChangelogSection }) {
  const { label, cls, accent, bg } = TAG_META[section.tag];
  return (
    <details
      open
      className="group rounded-md border border-border py-2.5 pr-3 pl-3.5"
      style={{
        borderLeft: `2px solid ${accent}`,
        background: `linear-gradient(90deg, color-mix(in srgb, ${accent} 8%, transparent), transparent 60%)`,
      }}
    >
      <summary className="flex cursor-pointer list-none items-center gap-2 group-open:mb-2 [&::-webkit-details-marker]:hidden">
        <span
          aria-hidden
        ...
        <span
          className={`rounded-[3px] border px-2 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-[0.18em] ${cls}`}
          style={{ background: bg }}
        >
          {label}
        </span>
        ...
        <span
          ...
          style={{
            background: `linear-gradient(90deg, color-mix(in srgb, ${accent} 35%, transparent), transparent)`,
          }}
        />
      </summary>
      <ul className="space-y-1.5 text-[13px] text-text">
        {section.items.map((it, i) => (
          <li key={i} className="flex gap-2.5 leading-relaxed">
            <span
              aria-hidden
              className="mt-2 inline-block h-1 w-1 shrink-0 rotate-45"
