...
export default function ProcsPanel() {
  ...
  const itemProcs = useMemo(() => itemProcRows(inventory), [inventory])
  ...
  const activeProcCount =
    ...
    itemProcs.filter((r) => !!procToggles[r.toggleKey]).length
  const totalProcCount =
    procSkills.length + subskillProcs.length + itemProcs.length
        ...
        <>
          ...
          {itemProcs.length > 0 && (
            <>
              ...
              <ul className="space-y-2">
                {itemProcs.map((row) => {
                  const checked = !!procToggles[row.toggleKey]
                  return (
                    <li key={row.toggleKey}>
                      <label
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-2">
                          <input
                            ...
                            checked={checked}
                            onChange={(e) => {
                              setProcToggle(row.toggleKey, e.target.checked)
                            }}
                          />
                          <span className="min-w-0">
                            <div
