...
export type CharmCell = readonly [number, number]

const BASE_BLOCKED_CELLS: ReadonlyArray<CharmCell> = [
  [3, 0],
  [3, 2],
  [7, 2],
]
// the in-game unlockable cell — toggled via settings.extraCharmSlot
export const CHARM_EXTRA_SLOT_CELL: CharmCell = [7, 0]

export function charmBlockedCells(
  extraSlotUnlocked: boolean,
): ReadonlyArray<CharmCell> {
  return extraSlotUnlocked
    ? BASE_BLOCKED_CELLS
    : [...BASE_BLOCKED_CELLS, CHARM_EXTRA_SLOT_CELL]
}
...
function buildInitialOccupancy(
  blockedCells: ReadonlyArray<CharmCell>,
): boolean[] {
...
export function packCharms(
  charms: { slotKey: SlotKey; w: number; h: number }[],
  blockedCells: ReadonlyArray<CharmCell> = charmBlockedCells(true),
  ...
  const usable = CHARM_GRID_ROWS * CHARM_GRID_COLS - blockedCells.length
  const totalArea = sorted.reduce((acc, c) => acc + c.w * c.h, 0)

  if (totalArea <= usable) {
    const occupancy = buildInitialOccupancy(blockedCells)
