...
const AUGMENT_ICONS = import.meta.glob<string>('../../../assets/augments/*.png', {
  eager: true,
  query: '?url',
  import: 'default',
})
const AUGMENT_ICON_BY_ID: Record<string, string> = {}
for (const [p, url] of Object.entries(AUGMENT_ICONS)) {
  const id = (p.split('/').pop() ?? '').replace(/\.png$/i, '')
  AUGMENT_ICON_BY_ID[id] = url
}
export function augmentIconForId(id: string): string | undefined {
  return AUGMENT_ICON_BY_ID[id]
}
