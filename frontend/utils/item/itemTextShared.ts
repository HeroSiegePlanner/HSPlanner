...
import { displayValuesNative } from '../calc/bridge'
import type {
  AffixValueRequest,
  DisplayValuesOutput,
  ScaledValueRequest,
...
export function formatCustomValue(format: 'flat' | 'percent', value: number): string {
  const suffix = format === 'percent' ? '%' : ''
  const signChar = value >= 0 ? '+' : ''
  const num = Number.isInteger(value) ? value : Math.round(value * 100) / 100
  return `${signChar}${num}${suffix}`
}
